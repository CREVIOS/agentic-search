...
impl SpanCache {
    ...
    /// Get-or-build. Builds the index synchronously on miss. Always
    /// hashes `bytes` itself so the cache key reflects the bytes the
    /// index was actually built from — no foot-gun for callers
    /// passing a wrong hash and poisoning the cache with a
    /// mis-attributed `ContainerIndex`.
